        ...
        switch (i) {
            case 0: // UP
                if (snake.m_headPos.m_y > 0) {
                    ...
                    if (!neighbor.m_wall && !neighbor.m_snake && !neighbor.m_longestUsed) {
...
size_t bfs(CPos start, CMap& map){
    ...
    while(!q.empty()){
        ...
        for(enumMove move : moves){
            ...
            if(!neighbor.m_wall && !neighbor.m_snake && !neighbor.m_bfsOpened){
...
bool getAvailability(CMap& map, CSnake& snake, enumMove move){
    CPos nextpos;
    if(move == UP) nextpos = CPos(snake.m_headPos.m_x, snake.m_headPos.m_y - 1);
    else if(move == DOWN) nextpos = CPos(snake.m_headPos.m_x, snake.m_headPos.m_y + 1);
    else if(move == RIGHT) nextpos = CPos(snake.m_headPos.m_x + 1, snake.m_headPos.m_y);
    else nextpos = CPos(snake.m_headPos.m_x - 1, snake.m_headPos.m_y);

    size_t countTotal = 0;

    for(size_t i = 0; i < map.m_height; i++) {
        for (size_t j = 0; j < map.m_width; j++) {
            auto &neighbor = map.m_map[i][j];
            if (!neighbor.m_wall && !neighbor.m_snake )
                countTotal++;
        }
    }

    size_t accessible = bfs(nextpos, map);

    if( 0.8 * (double)countTotal < (double)accessible)
        return true;
    return false;
}

enumMove  astar_search(CMap& map, CSnake& snake){
            ...
            switch (i) {
                case 0: // UP
                    if (field.m_y > 0) {
                        ...
                        if (!neighbor.m_wall && !neighbor.m_opened && !neighbor.m_snake && !neighbor.m_longestUsed ) {
                            if( snake.m_behave && getAvailability(map, snake, UP)) break;
                ...
                case 1: // DOWN
                    if (field.m_y + 1 < map.m_map.size()) {
                        ...
                        if (!neighbor.m_wall && !neighbor.m_opened && !neighbor.m_snake && !neighbor.m_longestUsed ) {
                            if( snake.m_behave && getAvailability(map, snake, DOWN)) break;
                            pq.emplace(make_pair(make_pair(neighbor.m_pos, length+1), distance(neighbor.m_pos, field_end, length+1)));
                            neighbor.m_opened = true;
                            prev[neighbor.m_pos] = field;
                        }
                ...
                case 2: // RIGHT
                    if (field.m_x + 1 < map.m_map[0].size()) {
                        ...
                        if (!neighbor.m_wall && !neighbor.m_opened && !neighbor.m_snake && !neighbor.m_longestUsed ) {
                            if( snake.m_behave && getAvailability(map, snake, RIGHT)) break;
                            pq.emplace(make_pair(make_pair(neighbor.m_pos, length+1), distance(neighbor.m_pos, field_end, length+1)));
                            neighbor.m_opened = true;
                            prev[neighbor.m_pos] = field;
                        }
                ...
                case 3: // LEFT
                    if (field.m_x > 0) {
                        ...
                        if (!neighbor.m_wall && !neighbor.m_opened && !neighbor.m_snake && !neighbor.m_longestUsed ) {
                            if( snake.m_behave && getAvailability(map, snake, LEFT)) break;
                            pq.emplace(make_pair(make_pair(neighbor.m_pos, length+1), distance(neighbor.m_pos, field_end, length+1)));
                            neighbor.m_opened = true;
                            prev[neighbor.m_pos] = field;
                        }
